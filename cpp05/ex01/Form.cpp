...
Form::Form(std::string name, int signing_grade, int executing_grade)
    : m_name(name), m_signing_grade(signing_grade),
      m_executing_grade(executing_grade), m_is_signed(false) {
    check_grade(m_signing_grade);
    check_grade(m_executing_grade);
}

Form::Form(const Form& f)
    : m_name(f.m_name), m_signing_grade(f.m_signing_grade),
      m_executing_grade(f.m_executing_grade), m_is_signed(f.m_is_signed) {
    return;
}
...
std::ostream& operator<<(std::ostream& o, const Form& f) {
    if (!f.get_is_signed()) {
        std::cout << "Form " << f.get_name()
                  << " can be signed by a bureaucrat of grade "
                  << f.get_signing_grade()
                  << " and executed by bureaucrat of grade "
